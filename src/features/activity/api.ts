...
async function getActivityPostsByIds(
  ...
  const { supabase, universityId } = await getCurrentUserContext();
...
export async function getSavedPosts(): Promise<ActivityPost[]> {
  ...
  const postIds = bookmarks.map((bookmark: Pick<BookmarkRow, "post_id">) => bookmark.post_id);
...
export async function getFavoriteUsers(): Promise<ActivityFavoriteUser[]> {
  const { supabase, universityId, userId } = await getCurrentUserContext();
...
export async function getFavoriteUserStatus(userId: string): Promise<boolean> {
  const { supabase, userId: currentUserId } = await getCurrentUserContext();
...
export async function getBookmarkedPostIds(postIds: string[]): Promise<string[]> {
  if (postIds.length === 0) {
    return [];
  }
...
export async function togglePostBookmark(
  ...
  if (postError || !post) {
    throw new Error("저장할 수 없는 게시물입니다.");
  }
