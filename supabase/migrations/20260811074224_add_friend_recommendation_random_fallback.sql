-- 신호가 없는 신규 사용자도 추천을 받을 수 있도록 같은 학교 후보를
-- seed 기반으로 섞어 채운다. 신호 후보는 tier로 랜덤 후보보다 항상 앞선다.

DROP FUNCTION IF EXISTS public.get_friend_recommendations(integer);

CREATE FUNCTION public.get_friend_recommendations(
