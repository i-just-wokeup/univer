...
-- 규칙: 최근 30일 내 · 전체공개 · 내 글 제외 · 차단 제외 · 미디어 있는 글 · 한 사람 최대 2개.
-- 핫스코어 = ln(1 + 좋아요 + 댓글*2) - 경과시간(h)/recency_hours
--   · 참여도는 로그로(도배 방지), 참여 없으면 0에서 시작. 최근일수록 감점 적음.
--   · recency_hours 클수록 인기 우위, 작을수록 최근 우위(기본 240 = 인기6:최근4 느낌).
