...
- **앱 제한 미디어 권한 안내** — 사진·영상 보관함이 `선택한 항목만 허용` 상태일 때 그리드 마지막에 기존 빈 상태와 같은 세로형 안내와 `권한 다시 선택` 버튼을 표시했다. Android는 읽기 권한을 다시 요청해 전체 허용 또는 항목 재선택이 가능한 최초 권한 흐름을 열고, iOS는 기존 항목 재선택 시트를 유지한다. 권한 창 종료 직후 상태를 직접 재조회하고 목록·페이지 커서를 초기화해 확대된 허용 범위를 즉시 반영하며, 허용 항목 0개의 기존 전체 화면 안내는 유지했다. 앱 tsc·대상 파일 lint 통과, 순수 JS라 재빌드 불필요.
