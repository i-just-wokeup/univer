...
### 완료
- `BUTTON_INVENTORY.md`의 기존 155행 표를 보존하고 호출부 기반 재분류 절을 추가했다. 배경→폭→위치→높이 계산 방식·동작 계약 순으로 검토했다. A46 중 선택기8/스토리·채팅 전용3 제외, 검토35 = 공용화 후보10묶음25곳 + 별도 계약10곳. 댓글 A3 포함, B/C 편입 없음.
- 추가 확인: 댓글 답글/본문 더보기의 터치 폭은 내용 폭이 아니라 stretch, 외부 공유는 시트 밖의 두 줄 고정 푸터, 프로필 관계에 비실행 상태 표시가 섞임, 추천 요청은 중첩 터치 이벤트 차단 필요. 공용 StateView의 채팅 사용을 확인해 후속 적용은 대상 호출부만 선택하도록 제한했다.
- 일반 채움은 minHeight+패딩, 중앙 안내는 기존 내용+패딩, 헤더 글자는 기존 슬롯 유지로 제안했다. 그룹별 치수·변화량과 글꼴 실측이 필요한 계산식을 기록했다. 제안 승인/구현은 아직 안 했으며 코드·마이그레이션 수정 없음.
- 기존 표 보존 및 A46 전체의 중복 없는 분류 합계 검증. 실기기 측정·타입 검사·빌드·배포는 하지 않았다(문서만 변경).

### 다음
- 디자이너 승인 후 범위 내 버튼만 공용화. 제외 화면에 대한 공용 컴포넌트 영향 차단, Android/iOS 글자 확대·375px 및 패딩형 실제 높이 확인 필요.

## 2026-09-17

### 완료
- 앱 버튼 전수 조사: `docs/BUTTON_INVENTORY.md`에 93개 파일의 실제 터치 구현 155곳(Pressable 152 + 인라인 Text 링크 3)을 기록했다. A 글자형 46(선택기 8 포함), B 아이콘형 45, C 복합/기타 64. 반복 목록과 공용 컴포넌트 호출은 중복 집계하지 않았다.
- A의 고정 높이 9종/minHeight 6종/반경 10종/pressed opacity 7종을 분리했다. 전체 pressed opacity는 11종이며, 상태 무관 숫자 opacity 22종과 구분했다. theme.radius 소비는 기존 제한 권한 안내 1곳이었다.
- 기존 114는 과거 스타일 정의 집계이며 당시 포함 조건은 확인 불가. 현재 button 이름 객체형 스타일 정의는 106개로, 비활성/눌림·스켈레톤·미사용 정의가 섞여 실제 버튼 수와 다르다. 고정44/작은 알약32/outline 제안을 현재 목록과 대조하고 5개 역할군을 제안했다.
- 의도 주석·DECISIONS의 7월 축소와 모달/제스처 예외를 읽고 문서에 표시했다. 코드·스타일·마이그레이션 수정 없음. 소스 정적 조사이며 실기기 크기/배포 버전 확인은 하지 않았다.

### 다음
- 디자이너와 실행 버튼 38곳 및 선택기 8곳을 분리해 규격 승인. 공용 Button 구현과 값 변경은 후속 작업이다.
...
- 자기 좋아요는 가능한 구조(실제 33건), 댓글은 deleted_at 없는 hard delete임을 확인하고 자기 반응을 제외했다. 실측: 무반응 0/99개(0%), 첫 반응 중앙값 1.8시간, 재작성 13/17명(76.5%), 이번 주 대표 4명. D1 47.6% 유지, D7 33.3→61.9%, D30 20→40%.
- 타입 검사/변경 파일 ESLint/웹 빌드 통과. 읽기 전용 SQL fixture로 자기 반응·스토리 좋아요·삭제 글 제외, 빈 모집단, KST 1/7/30일 자정 경계, 168시간 경계와 미성숙 코호트 제외를 검증했다. 파서 정상/null/누락/잘못된 배열·날짜·숫자 거부 검증 통과.
- 기존 파워유저 그래프 최소 너비가 모바일 화면 전체를 늘리는 문제를 성장 컴포넌트의 minmax(0,1fr) 그리드로 제한했다. 새 주간 막대는 공통 왼쪽 기준선·최대값·zinc 트랙·분리 축을 사용한다.
...
### 완료
- 웹 관리자 운영/성장 탭 추가. 성장 대표 WAU, 유입/행동별 활성화 퍼널/잔존/1~30일 활동 분포를 제공한다. `get_admin_growth_stats()` SQL 마이그레이션과 타입·엄격한 응답 파서 추가. Promise.allSettled로 기존 KPI와 성장 조회 실패를 분리하며 실패를 0으로 표시하지 않는다. 앱은 수정하지 않았다.
- 루트 빌드와 변경 파일 ESLint 통과. 원격 읽기 전용 쿼리 및 KST/빈 데이터/중복/잔존/부활 경계 검산. 탈퇴자 1명 제외로 제공된 22명 대신 21명이며, 포함하면 검산값 재현. 끈끈함은 오늘 DAU/MAU와 7일 평균 DAU/MAU를 구분했다. 상세: `docs/ADMIN_GROWTH.md`.
- 원격 마이그레이션 적용·웹 배포·커밋은 하지 않았다. 다음: 마이그레이션 적용 후 실제 관리자/일반 계정 권한 검증 및 웹 배포.
- 브라우저 테스트 응답으로 탭 전환/부분 실패 분리 확인, 1280px·375px 가로 넘침 없음. 기존 개발 서버의 비로그인 `/admin` 접근은 로그인으로 리다이렉트됨을 확인.
...
- 🍎 **iOS 실기기 첫 검증 — 카메라 결함 3건 수정** (iPhone 12, 개발 빌드). 그동안 iOS는 1.0.0 빌드뿐이고 현재 코드는 1.0.2라 OTA가 닿지 않아 한 번도 확인하지 못했던 영역이다.
  - **닫기 버튼이 노치에 가려 안 눌림** — 게시물 카메라만 `Modal` 안에 있는데, Modal 은 네이티브 계층이 분리돼 **최상위 `SafeAreaProvider` 가 잰 여백이 넘어오지 않는다.** 상단 `SafeAreaView` 여백이 0이 됐다. Modal 안에 Provider 를 다시 뒀다. 스토리 카메라는 Modal 이 아니라 멀쩡했다.
  - **전후면 전환 시 로딩이 안 사라짐** — 전환할 때 `isCameraReady` 를 false 로 되돌리고 `onCameraReady` 를 기다리는데, **iOS 는 `facing` 만 바뀔 때 뷰가 그대로라 다시 부르지 않는다.** 로딩이 영영 남고 촬영 버튼이 잠겼다. 초기화를 Android 에서만 하도록 바꿨다(CameraX 는 카메라를 다시 붙이며 한 번 더 부른다).
  - 🔴 **미리보기·사진 비율이 Android 와 다름 + 화질이 낮음** — `ratio` 는 Android 전용이라 iOS 미리보기가 화면을 꽉 채웠는데, `takePictureAsync` 가 **"scaled to match the preview"** 라서 **사진까지 화면 비율로 찍혔다.** 실측 `888x1920`(207만 화소) — `1080x1920` 원본에서 가로를 192px 잘라낸 값이다.
    - ① 미리보기를 iOS 에서만 9:16 박스로 묶어 사진이 따라오게 하고, ② 그래도 1920x1080 세션이라 화소가 낮아 `pictureSize="3840x2160"` 을 지정했다.
    - 아이폰 12 에서 쓸 수 있는 값: `3840x2160` · `1920x1080` · `1280x720` · `640x480` · `352x288` · `Photo` · `High` · `Medium` · `Low`. **`"Photo"` 가 1200만으로 더 높지만 4:3 이라 Android(16:9)와 프레이밍이 달라져 쓰지 않았다.**
    - 결과: **iOS 2160x3840(830만) · Android 2252x4000(900만)** 으로 비율이 같아지고 화소는 4배가 됐다.
  - **전부 iOS 분기로만 처리했다.** Android 는 실기기에서 해상도·버튼 위치·전환 동작 변화 없음을 확인했다.
  - 커밋 `e947180` · `896618c` · `f3a9648`. 순수 JS라 재빌드 불필요.
  - 🔸 **교훈**: 9/11 iOS 감사에서 "유일한 차이는 카메라 미리보기 프레이밍"이라고 적었는데, 실제로는 **프레이밍이 사진 해상도까지 끌고 내려갔다.** 문서를 읽고 추론한 범위와 실기기에서 드러난 범위가 달랐다.
- **공용 `Icon` 컴포넌트와 아이콘 토큰 추가** — 아이콘 83곳에서 색·크기·굵기를 매번 손으로 적어 값이 갈렸다(같은 화면 안에서도 카메라 조작 3개가 `27/2.5` · `24/2.4` · `28/2.2`). `iconSize`(xs14 sm18 md22 lg26 xl30)·`iconStroke`(thin2 regular2.4 bold2.8) 토큰과 아이콘 36개를 모은 `components/common/Icon.tsx` 를 추가했다. `tone` 으로 색을 역할로 묶어 같은 "미디어 위"인데 `colors.white` 31곳 / `onMediaGlyph` 15곳으로 갈려 있던 것도 합친다.
  - 릴스의 `1.8` 은 `thin`(2)과 눈에 띄는 차이가 없어 흡수하기로 했다(사용자 확인).
  - **이번엔 추가만 했다.** 호출부 교체는 iOS 검증 뒤로 미뤘다 — 카메라 화면이 교체 대상이자 검증 대상이라 같이 바꾸면 문제 원인이 섞인다. 커밋 `b3b16d9`.

### 다음
- 아이콘 호출부 50개 교체 (정리 대상 37곳 / 23파일). `grep "의도:"` 로 건드리면 안 되는 값을 먼저 확인할 것.
- 바텀시트 6개 통일 → 공용 `Button` → 입력칸 순서.
- iOS 는 아직 1.0.0 스토어 빌드뿐이다. 위 수정을 내보내려면 **iOS 빌드·제출이 필요**하다.

## 2026-09-11

### 완료
- **홈피드 신규 글 밴드를 6시간 → 48시간으로 확대** — 밴드 1을 벗어난 글은 밴드 2의 핫스코어 경쟁으로 떨어지는데, 핫스코어는 `(1 + 좋아요 + 댓글x2) x 0.5^(나이/5일)` 이라 **좋아요 하나가 점수를 배로 만든다.** 반응이 아직 없는 새 글은 6시간이 지나는 순간 옛날 인기 글에 밀려 묻혔다. 새벽에 올린 글이 대표적이다.
  - 밴드 1 조건에 **이미 "안 봤음"이 들어 있어**, 창을 넓혀도 내가 본 글은 자동으로 빠진다. 별도 도달 수 계산이나 설정 테이블 없이 기존 구조가 "안 본 사람에게만 계속 보인다"를 해준다. 맨 위에 영구히 박히는 문제도 없다.
  - 실측: 활동 상위 5명 기준 밴드 1 글 수가 **2개 → 3~7개**. 하루 게시물이 약 5개라 이틀치에 해당한다.
  - `CREATE OR REPLACE` 사용(시그니처 불변) → **DROP 하지 않아 `anon`/`authenticated` 실행 권한이 유지됐다.** 적용 후 함수 1개·기본값 48·양쪽 권한 모두 확인.
  - 마이그레이션 `20260911100000_feed_fresh_window_48h`, 원격 적용·검증 완료. **앱 수정·OTA 불필요** — 앱은 `p_fresh_hours` 를 넘기지 않으므로 1.0.1·1.0.2·개발 빌드 전부에 다음 피드 조회부터 동시에 적용된다(앱 피드 캐시 5분, 당겨서 새로고침하면 즉시).
  - 🔸 **확인할 것**: 글당 평균 도달 6.4명이 며칠 뒤 오르는지. 안 오르면 되돌리거나 시간을 조정한다(숫자 두 곳만 6으로).
- **`docs/FEED_RANKING.md` 신설** — 밴드 설명이 작업일지 8군데에 흩어져 있어 **현재 상태를 알려면 마이그레이션 10개를 거슬러 읽어야 했다.** "지금 어떻게 도는가"만 적는 문서를 따로 만들었다(바뀌면 이 파일을 고치고 쌓지 않는다). 홈 밴드 0~4·탐색 점수식·열람 기록 방식·실측 수치·확정된 계획·**안 하기로 한 것 9개**·틱톡/X/인스타 조사 결과·관련 파일 목록.
- **iOS 카메라 권한 문구 수정** — `"스토리를 촬영하려면 …"` 이었는데 9/3에 **게시물 작성에도 카메라를 넣어** 실제 사용처와 어긋났다. 애플은 이 문구와 실사용의 일치를 심사한다. `"사진·영상을 촬영해 게시물이나 스토리에 올리려면 …"` 으로 교체(`app.json`).
  - ⚠️ **안드로이드는 이 값을 쓰지 않는다.** 안드로이드 권한 창 문구는 OS 고정이라 앱이 못 바꾸고, 매니페스트에는 권한 이름만 들어간다(검색 결과 0건 확인). 안드로이드에서 우리 문구가 보이는 곳은 요청 **전에** 뜨는 앱 안 설명 화면(`CameraPermissionView`)인데, 이미 `"사진을 촬영하려면 …"` 이라 스토리·게시물 양쪽에 맞다. 수정 불필요.
  - 반영은 **다음 iOS 빌드부터.** 현재 iOS 빌드 없음.

### 점검
- **앱 타입 검사 통과** — `tsc --noEmit` 오류 0. 다만 `package.json` 에 스크립트 등록이 없어 `node node_modules/typescript/bin/tsc --noEmit` 를 직접 쳐야 한다. **등록 필요.**
- **위치 권한 미사용 확인** — `expo-location` 미설치, 관련 코드 0건. iOS에서 문구 없는 권한 요청은 앱이 즉사하는데 **해당 없음.** 연락처·캘린더·블루투스 등 다른 민감 권한도 없다.
- 🔸 **안드로이드 매니페스트에 `SYSTEM_ALERT_WINDOW`(다른 앱 위에 표시)가 있다.** 우리가 넣은 것이 아니라 라이브러리가 자동 추가한 것으로 보이나, 구글이 가끔 문제 삼는 권한이다. **프로덕션 심사 전에 출처 확인 필요.**

### 다음
- **iOS는 아이폰 확보 후.** 검사로 잡을 수 없는 것들이라 실기기가 필요하다 — `ratio="16:9"` 가 iOS에서 무시돼 미리보기 프레이밍이 다름, 카메라 미리보기 잘림, 제스처 반응 차이. `npx expo prebuild --platform ios` 는 맥 없이도 설정 오류를 미리 볼 수 있으나 **아직 돌리지 않았다**(`ios/` 폴더 없음, CNG 방식).
- 새 글 도달 보장(A)·묻힌 글 구제(B)·`feed_settings`(C)는 설계만 확정. 노션 백로그 2026-09-11 항목과 `docs/FEED_RANKING.md` 6절 참고.

## 2026-09-09

### 완료
- **안드로이드 1.0.2 (versionCode 5) 비공개 테스트 출시** — 스토어 빌드가 8/28 versionCode 3에 머물러 있어 그동안 OTA로 나간 것들(카메라·연속 스크롤·사진 순서)이 **새로 설치하는 사람에게는 하나도 안 들어갔다.** 프로덕션 액세스 신청 전에 스토어 빌드를 최신으로 맞췄다. 제출 ID 6, 9/9 10:09 `출시됨`.
  - 빌드 전 AAB를 직접 검사: 서명 `60:E5:30:…`(6/24 원래 키) · 패키지 `com.univer.app` · 버전 `1.0.2`
  - 🔸 업로드는 전날 됐는데 출시 절차를 안 끝내 `5 버전 코드는 이미 사용되었습니다` 오류가 났다. **`라이브러리에서 추가`** 로 기존 번들을 붙이면 된다. 9/6에 이어 두 번째로 **"검토를 위해 변경사항 제출"을 빠뜨렸다.**
  - ⚠️ 버전이 갈려 **1.0.1을 쓰는 테스터는 OTA를 더 못 받는다.** Play 업데이트가 필요하다.
- **`app_release.latest_version` 을 1.0.2로 갱신** — 9/4 테스트 후 1.0.0으로 되돌려둔 채였다. 그래서 앱이 "내가 최신"이라 판단해 **업데이트 안내가 아예 안 떴다.** 실제로 스토어에 1.0.2가 올라간 것을 확인하고 올렸다. 앱은 하루 한 번 확인하므로 사람마다 **최대 하루까지 늦게** 뜬다.
- 🔴 **접속 지표가 실제와 달랐다 — 계산 방식을 바꿨다.**
  - 화면에 `7일 이상 안 옴 19명` 으로 나왔는데, 실측하니 **최근 7일에 17명이 피드를 봤고 14명이 글을 썼다.**
  - 원인: `last_seen_at` 은 **앱이 스스로 보고해야** 채워지는데 그 기능이 9/7 OTA로 나가 아직 2명만 보고했다. **"안 온 사람"과 "아직 보고 안 한 사람"이 한 칸에 섞여** 있었다.
  - 수정: 이미 쌓이는 기록으로 센다 — `post_impressions`(피드 열람) + `posts`·`comments`·`post_likes` 의 최근 시각과 `last_seen_at` 중 **가장 최근 값**을 그 사람의 마지막 활동으로 본다. 앱 업데이트를 기다릴 필요가 없다.
  - 결과: 오늘 2→**6명**, 7일 2→**16명**, 안 옴 19→**4명**, 활동 기록 없음 **1명**으로 정정.
  - 앱 버전은 앱이 보고해야만 알 수 있으므로 `versionUnknown` 을 따로 내보내 버전 카드에 "아직 버전을 알 수 없는 인원"으로 표시했다.
  - 마이그레이션 `20260909110000_admin_ops_activity_from_impressions`
- 🔸 **Play Console 알림이 다른 계정으로 열리던 것** — *"9/30까지 앱을 등록하세요"* 링크를 누르면 `simsim020304` 로 인증하라고 나왔다. 콘솔 주소가 `console/u/1/…` 인데 알림 링크에 계정 번호가 없어 **첫 번째 계정(`u/0`)** 으로 열린 것이다. `u/1` 로 열면 이미 인증 완료 상태다. **앞으로 Play Console 링크는 `u/1` 인지 확인할 것.**

## 2026-09-08

### 완료
- **관리자 운영 지표 오류 수정** — 어제 붙인 카드 3개가 틀린 숫자를 보여주고 있었다. 코드 리뷰에서 나왔고 DB로 직접 확인한 뒤 고쳤다.
  - 🔴 **"영상 처리"가 사진까지 세고 있었다.** `완료 166건` 중 **사진이 124건**, 영상은 42건뿐이었다. `processing_status` 가 `not null default 'ready'`(`20260701051431`)라 모든 사진이 `ready` 로 들어간다. `type = 'video'` 로 거르고 삭제된 게시물의 미디어도 뺐다 → **18건**으로 정정.
  - 🔴 **"앱 버전"이 `알 수 없음` 으로 도배됐다.** 배포 전 사용자가 대부분이라 그 한 줄이 목록 맨 위를 차지해 실제 버전이 밀려났다. 기록 없는 인원은 접속 카드(`activity.unknown`)에 이미 있으므로 목록에서 제외했다.
  - 🔴 **새 지표가 실패하면 기존 KPI까지 0으로 보였다.** `Promise.all` 로 묶여 있어 한쪽이 죽으면 가입·게시물·댓글이 전부 0이 되고, `isLoading` 은 `finally` 에서 풀려 **0이 실제 값처럼 렌더**됐다. `Promise.allSettled` 로 분리해 한쪽이 실패해도 다른 쪽은 보여준다.
  - 🔴 **오류가 나도 카드가 그려졌다.** "영상 실패 0건"으로 보여 운영자가 문제없다고 읽는다. 운영 지표를 못 읽었으면 카드를 아예 그리지 않도록 했다.
  - 🔴 **앱을 완전히 끄지 않으면 접속 기록이 안 남았다.** `recordedKeyRef` 가 프로세스 수명 내내 유지돼 콜드 스타트에서만 기록했다. 백그라운드에 두고 매일 쓰는 사람이 "7일 이상 안 옴"으로 잡혔다 — 접속자 수를 보려고 만든 기능인데 그 숫자가 틀어진다. `AppState` 가 `active` 로 돌아올 때도 기록하되 **1시간에 한 번**으로 제한했다.
  - 🔸 `오늘` 라벨은 실제 계산(최근 24시간)에 맞춰 **"최근 24시간"** 으로 바꿨다.
  - 마이그레이션 `20260908090000_fix_admin_ops_stats_video_and_versions`, 원격 적용·검증 완료. 앱·웹 tsc 통과.

## 2026-09-07

### 완료
- **앱 버전·마지막 접속 기록 + 관리자 운영 지표** — 지금 DB에 앱 버전을 남기는 곳이 아예 없어서 "테스터가 새 버전으로 올라왔나"를 Play Console을 열어야 알 수 있었고 **iOS는 그것도 안 됐다.** `users` 에 `app_version` · `app_platform` · `last_seen_at` 을 추가하고, 앱이 켜질 때 계정당 한 번 자기 버전을 보낸다.
  - `users` 는 민감 컬럼을 트리거로 보호하므로 직접 UPDATE 하지 않고 **이 세 칸만 고치는 `record_app_session(p_app_version, p_app_platform)` RPC**(SECURITY DEFINER, `auth.uid()` 본인 행만)로 갱신한다. 실패해도 앱 사용을 막지 않으며 다음 실행에서 다시 시도한다.
  - **관리자 대시보드에 카드 3개 추가** (`get_admin_ops_stats`, admin 전용):
    - **접속** — 오늘 / 최근 7일 / 7일 이상 안 옴 / 탈퇴. 지금까지 Expo·Play Console을 열어야 보던 값이다(9/1 15명 → 9/6 2명을 오늘 Expo에서 확인했다)
    - **앱 버전** — 플랫폼·버전별 인원. 누가 업데이트했는지 바로 보인다
    - **영상 처리** — 실패 / 처리 중 / 완료 + 승격 신청 대기. **인코딩 실패는 지금까지 아무도 몰랐다.** 올린 사람은 올라간 줄 안다
  - 🔸 **기존 대시보드에 없던 것들**을 훑어 고른 것이다. 채팅·크루·팔로우 수, 릴스 시청은 데이터는 있으나 이번에는 넣지 않았다(숫자만 보는 것이라 급하지 않다).
  - 🔸 **OTA로 나가면 지금 1.0.1 쓰는 사람부터 바로 쌓인다.** 그전 사용자는 `알 수 없음` 으로 잡히고, 화면에도 "이 기능이 들어간 버전으로 앱을 켜야 잡힌다"고 안내한다.
  - 마이그레이션 `20260907120000_user_app_session_and_admin_ops_stats`, 원격 적용·검증 완료(관리자 계정으로 RPC 호출 확인, `record_app_session` 실제 기록 확인 후 테스트 값 정리). 앱·웹 tsc 통과.
- **앱 게시물 사진 순서 바꾸기(끌어서 옮기기)** — 사진을 여러 장 고르면 누른 순서로 고정돼 바꿀 방법이 없었다. **첫 사진이 게시물 비율을 정하므로**(`detectAspectRatio`) 순서는 취향 문제가 아니다. 썸네일을 길게 누르면 반투명(85%)으로 커지며 떠올라 손가락을 따라오고, 나머지는 흐려지며(60%) 옆으로 비켜 빈자리를 만든다. 손을 떼면 그 자리로 들어가고 첫 자리가 바뀌면 비율을 다시 계산한다. 집는 순간 가벼운 진동을 준다.
  - 🔴 **가장 큰 위험은 바텀시트 충돌이었다.** 이 썸네일 줄이 시트 손잡이 영역이라 위아래로 끌면 시트가 열리고 닫힌다. **해결은 `activateAfterLongPress(300)` 하나였다.** 설치된 RNGH 안드로이드 소스(`PanGestureHandler.kt`)에서 확인한 동작 — `activateDelayed = Runnable { activate() }` 로 300ms 뒤 조건 없이 활성화되고, 대기 중 손가락이 조금이라도 움직이면 `shouldFail()` 이 true가 되어 **제스처가 실패한다.** 그래서 짧은 탭·가로 스크롤·세로 시트 끌기가 전부 그대로 넘어간다.
  - 🔸 **새 라이브러리를 쓰지 않았다.** `package.json` 무변경 → 네이티브 재빌드 불필요, OTA 배포 가능. 후보(reanimated-dnd·draggable-flatlist·reorderable-list·drax)를 조사했으나 우리 경우가 단순하고(최대 10장·고정 크기·한 줄), 과거 `@gorhom/bottom-sheet` + Reanimated 4 조합에서 조용히 안 되던 사고가 있어 직접 구현했다.
  - 🔸 목록은 `FlatList` 대신 RNGH `ScrollView` + 전체 렌더로 바꿨다(최대 10장이라 가상화 이득이 없고 위치 계산이 단순해진다). 끄는 동안에는 `scrollEnabled={false}`.
  - 🔸 **남은 것 — 6장 이상일 때 가장자리 자동 스크롤.** 한 칸 60pt(썸네일 52 + 간격 8)라 **6장부터 화면(375) 밖으로 넘어가는데**, 끄는 동안 목록 스크롤을 막아둬서 **맨 끝으로 옮길 수 없다.** 5장까지는 정상. 스크롤한 양만큼 위치 계산을 보정해야 해서 틀리면 엉뚱한 자리에 놓인다. 백로그에 기록.
  - 가지 `feat/photo-reorder` 에서 단계별로 만들고 `main` 에 합쳤다. 커밋 `1fbe1f0`·`458d4b1`·`386b739`.
- **OTA 배포** — 위 세 작업(카메라 저장, 연속 스크롤, 사진 순서 바꾸기)을 `--branch production --platform android` 로 내보냈다.
  - 🔸 `--platform android` 를 빼면 웹 번들까지 만들려다 실패한다(`react-native-web` 미설치). `app.json` 에서 웹을 빼는 정리는 아직 안 했다.
- **앱 탐색·프로필 게시물 연속 스크롤** — 탐색 사진과 프로필의 사진·ready 영상을 누르면 선택 글을 첫 항목으로 두고 이후 게시물을 홈과 같은 `FeedPostCard`로 이어 보는 `/posts` 화면을 추가했다. 탐색 화면이 보유한 메모리 페이지 캐시는 TTL이 지났어도 연속 화면에서 그대로 재사용하고, 끝에 도달하면 기존 offset 페이지네이션으로 다음 탐색 페이지를 이어 받는다. 프로필은 기존 `getProfilePosts(userId)`의 전체 목록을 그대로 사용하며 processing·failed 영상은 제외한다. 목록에서는 60% 이상 보이는 영상 하나만 재생하고, 영상 영역을 누르면 해당 작성자의 ready 영상만 선택 영상부터 `created_at` 최신순으로 이어지는 릴스를 연다. 좋아요·댓글·저장·공유·신고·차단·삭제와 댓글/공유 시트를 재사용하며, 이 화면에는 홈의 `post_impressions` 기록을 연결하지 않았다. 탐색 영상은 기존 전체 랭킹 릴스 진입을 유지했다.
- **앱 카메라 촬영 원본 `unip` 보관함 저장** — 공통 촬영 성공 직후 저장 전용 사진 권한을 확인·요청한다. 첫 사진은 촬영 파일 URI를 `initialAssetLocalUri`로 넘겨 신규 `unip` 앨범 안에 바로 만들고, 이후 사진도 생성 시점부터 해당 앨범을 지정해 중복과 Android 이동 확인창을 피한다. 앨범 조회·생성 실패 시 기본 위치 저장으로 폴백하며 권한 거부·용량 부족 등 최종 실패도 스토리·게시물 작성 흐름을 막지 않는다. iOS 저장 권한 문구도 앱 설정에 추가했다.
  - 🔸 **처음에 확인창이 떴다.** 폴더가 없을 때 `createAssetAsync(uri)` 로 DCIM에 먼저 만들고 `createAlbumAsync(..., asset, false)` 로 옮기게 짜서, 안드로이드 11+가 **"이 사진을 수정하도록 허용하시겠습니까?"** 를 띄웠다. 기존 사진을 옮기는 동작이라 그렇다. `initialAssetLocalUri` 로 **파일 경로를 직접 넘겨 폴더 안에 바로 만들도록** 바꾸니 사라졌다. 공식 문서가 "앨범으로 옮기지 말고 생성 시점에 앨범을 지정하라"고 명시한 그대로다.
  - 🔸 iOS 저장 권한(`NSPhotoLibraryAddUsageDescription`)은 **이미 들어가 있었다.** `@expo/config-plugins` 의 `applyPermissions` 가 값을 안 주면 플러그인 기본 문구를 넣는다(`infoPlist[key] = 준 값 || 기존 값 || 기본값`). 그래서 현재 배포된 iOS 빌드에서도 저장이 동작한다. 이번에 넣은 한국어 문구는 다음 네이티브 빌드부터 보인다.
  - 🔸 **스토리 미리보기 저장 버튼은 만들지 않았다.** 지금 꾸미기가 배경색뿐이라 저장할 결과물이 촬영 원본과 같다. 글씨·그림이 생기면 그때 꾸민 결과를 저장하는 버튼을 붙인다.
  - 🔸 간헐 관찰: 촬영 → `여러 장` 켜기 → 보관함 사진 탭 시 촬영 사진이 선택에서 빠지는 경우를 한 번 봤으나 재현되지 않았다. 다시 나오면 **선택 사진 줄에도 없는지** 확인할 것(그리드 표시 문제인지 실제 누락인지 갈린다).
- **앱 카메라 9:16 통일 · 셀카 좌우 반전 · 스토리 줌** — 세 가지를 공통 컴포넌트 한 곳에서 처리해 스토리·게시물에 함께 적용했다. 실기기 확인 완료.
  - **미리보기와 찍힌 사진이 달랐다.** 화면에 보이던 것보다 사진이 더 넓게 나왔다. 원인은 둘이었다. ① `ratio`를 안 주면 `previewView.scaleType`이 FILL이라 **미리보기가 좌우를 잘라서** 화면을 채운다. ② `buildResolutionSelector()`가 `HIGHEST_AVAILABLE_STRATEGY`로 떨어져 **미리보기와 촬영이 각자 크기를 골라** 담기는 범위 자체가 달라진다. 둘 다 설치된 expo-camera 소스에서 확인했다.
  - **`ratio="16:9"` 하나로 정리했다.** 미리보기와 촬영이 같은 모양을 쓰고 scaleType이 FIT으로 바뀌어 **보이는 그대로 찍힌다.** 세로로 들면 9:16이라 화면도 시원하게 찬다.
  - 🔸 **4:3을 먼저 시도했다가 되돌렸다.** 게시물이 4:5·1:1·16:9 중에 고르니 넓게 찍어두는 게 낫다고 판단했는데, ① 화면의 6할만 차지해 답답하고 ② 스토리는 촬영 후 조정 없이 9:16으로 잘려 올라가서 결과가 미리보기와 계속 어긋났다. **게시물은 자르기 화면에서 사용자가 조정하면 되므로** 둘 다 9:16으로 통일하는 쪽이 맞았다.
  - **셀카 좌우 반전** — `mirror`를 켰다. 소스(`ExpoCameraView.kt:301`)에서 `mirror && lensFacing == FRONT` 조건이라 **앞 카메라일 때, 찍힌 사진에만** 적용된다. 미리보기와 뒷 카메라에는 영향이 없다.
  - **스토리 카메라에 줌 추가** — 공통화 때 일부러 미뤄뒀던 것으로, 호출부에서 `enableZoom` 한 줄로 켰다.
  - 🔸 **남은 것**: 찍은 사진 갤러리 저장(다음 작업). iOS는 `app.json`에 `savePhotosPermission` 문구가 없어 **네이티브 빌드가 필요**하므로 안드로이드부터 OTA로 내보내고 iOS는 다음 빌드에 태운다.
  - 🔸 `ratio`는 **Android 전용** prop이다. iOS 프레이밍은 별도로 확인해야 한다(현재 iOS 테스터 없음).
- **앱 카메라 촬영 공통 구조 분리·동작 통일** — `appearance` 갈래와 레거시 색상 참조를 제거하고 권한 UI(`CameraPermissionView`), 조작 UI(`CameraControls`), 촬영·권한·줌 로직(`useCameraCapture`), 조립 화면(`CameraCaptureView`)을 각각 200줄 이하로 분리했다. 준비 전 셔터 차단, 촬영 중 전환 차단, 유효하지 않은 결과·실제 오류 표시, 전환 시 플래시·줌 초기화를 공통 적용하고 앞 카메라에서는 플래시 대신 같은 크기의 빈 슬롯을 둔다. 스토리 9:16 크롭·갤러리 사진/영상 선택과 게시물 `Modal`·Android `GestureHandlerRootView`는 호출부에 유지했으며 앱 tsc·변경 파일 ESLint를 통과했고 **실기기에서 스토리·게시물 카메라 회귀 확인 완료**.
  - 🔸 갈래를 걷어내면서 드러난 것: 두 화면의 차이는 설계가 아니라 **따로 만들다 어긋난 것**이었다. 여백 18/16, 36/32, 18/20, 아이콘 26/27처럼 눈에 안 보이는 수치 차이와, 스토리 쪽이 덜 안전한 동작(준비 전 셔터 허용, 실패를 조용히 넘김, 전환 시 플래시 유지)이 섞여 있었다. 전부 게시물 기준으로 통일했다.
  - 🔸 **새 파일이 생기면 Metro를 `--clear`로 다시 켤 것.** 분리 직후 `Unable to resolve "../../features/camera/useCameraCapture"`로 앱이 안 열렸는데, 파일은 정상으로 있었고 번들러가 못 본 것이었다.

## 2026-09-06

### 완료
- **앱 게시물 작성 카메라 촬영 연결** — 보관함 3열 그리드 첫 칸에 같은 크기의 카메라 버튼을 넣고(인스타 방식), 전체 화면 `PostCamera.tsx`를 새로 만들어 전후면 전환·플래시·핀치 줌·권한 요청/설정 이동을 붙였다. 촬영 원본의 URI와 크기를 기존 단일/여러 장 선택과 3종 비율 자르기 흐름에 그대로 태우고, `camera:` 자산은 iOS MediaLibrary 조회를 건너뛰도록 분리했다. **실기기에서 촬영·줌 확인 완료.** tsc·lint 통과. **재빌드 불필요(OTA)** — 현재 AAB에 `CAMERA`·`RECORD_AUDIO` 권한이 이미 선언돼 있다.
  - 🔴 **한 번 크게 돌아갔다. 원인은 하나였는데 증상이 둘로 보였다.**
    - 처음에 `GestureDetector`로 `CameraView`를 **직접 감쌌더니** 미리보기만 나오고 **촬영도 줌도 안 됐다.** 촬영 시 `Failed to capture image`.
    - **해결**: 카메라 위에 투명한 `View`를 덮고 손동작을 거기서 받도록 분리했더니 **둘 다 한 번에 살아났다.** 네이티브 카메라 뷰에 제스처 핸들러를 직접 붙이면 안 된다.
  - 🔸 **`Failed to capture image`는 원인을 알 수 없는 문구다.** 설치된 소스를 직접 확인했다 — `ExpoCameraView.kt:312`의 `onError(exception)`가 안드로이드가 준 `ImageCaptureException`을 **버리고** `CameraExceptions.ImageCaptureFailed()`(고정 문구 "Failed to capture image")만 던진다. 실제 사유(`ERROR_CAMERA_CLOSED` / `ERROR_CAPTURE_FAILED` 등)는 logcat에만 남는다. 다음에 이 문구를 보면 **바로 ADB 로그를 볼 것.**
  - 🔸 **왜 미리보기는 살아 있는데 촬영만 실패했나**: `createCamera()`가 `imageCaptureUseCase`를 **연결 전에 미리 만들어 두고** 그 뒤에 `cameraProvider.unbindAll()` → `bindToLifecycle` 을 한다. 그래서 연결이 끊겨도 촬영 도구 객체는 남아 있어 명령은 가고 실패만 한다. `unbindAll()` 때문에 **카메라가 두 개 뜨면 나중 것이 먼저 것을 죽인다** — 공식 문서도 "한 번에 하나의 미리보기만 활성화될 수 있다"고 명시한다.
  - 🔸 **줌은 Reanimated로 넣으면 안 된다.** 처음에 `useAnimatedProps` + `Animated.createAnimatedComponent(CameraView)`로 만들었는데, `CameraView`는 그 방식으로 prop을 받도록 되어 있지 않아 **New Architecture(Fabric)에서 에러 없이 조용히 무시된다.** 일반 React 상태 + `zoom` prop(0~1)으로 교체했다.
  - 🔸 조사에서 걷어낸 헛다리: [expo#28966](https://github.com/expo/expo/issues/28966) "CameraView 위 제스처 미인식"은 **iOS 전용이고 이미 수정**됐다(이슈 본문에 "Android에서는 인식된다"고 적혀 있음). RNGH 공식 문서의 "Modal 안에서는 `GestureHandlerRootView`로 감싸야 한다"(안드로이드에서 Modal이 RN 루트 뷰 아래에 없기 때문)는 조치도 넣어뒀으나, 이번 원인은 그게 아니었다.
- **안드로이드 1.0.1 (versionCode 4) 비공개 테스트 출시** — 마지막 업로드가 8/28 versionCode 3이라 14일 요건 유지를 위해 올렸다. 제출 ID 5, 9/6 13:09 제출 → `출시됨`.
  - **앱 버전을 1.0.0 → 1.0.1로 올렸다**(커밋 `9b6e040`). 스토어 앱이 1.0.0에 머물면 서버 `latest_version`과 항상 같아 업데이트 안내가 영원히 못 뜬다.
  - 🔸 그래서 **지금 1.0.0을 쓰는 테스터는 업데이트 안내를 못 본다.** 그 앱에는 기능 자체가 없고, 버전을 올리면서 OTA도 1.0.1 앱에만 가도록 갈렸다. 이 기능이 실제로 처음 작동하는 건 **1.0.2를 낼 때**다. Play 자동 업데이트로 대부분은 알아서 올라간다.
  - 🔸 Play 업로드 경고 `가독화 파일 없음`은 무시해도 된다. 두 AAB(8/28 통과분·오늘분)의 메타데이터를 직접 비교해 동일함을 확인했고, dex를 뜯어 클래스 이름 489개가 원본 그대로 읽히는 것도 확인했다. **R8/proguard를 아예 안 쓰고 있어 올릴 대조표가 없다.**
- 🔴 **프로덕션 AAB가 개발용 keystore로 서명돼 Play가 업로드를 거부했다 (해결, 커밋 `caf733c`)**
  - Play 오류: 기대 SHA1 `60:E5:30:…` / 실제 `CC:C8:8F:…`. 두 AAB의 인증서를 직접 뽑아 비교했고, 문제의 키가 **9/3 18:45에 생성**된 것을 확인했다 — 개발 빌드 패키지를 분리한 그 시각이다.
  - **원인**: `app.config.js`가 `EAS_BUILD_PROFILE`을 보는데 **이 값은 EAS 서버에서만 설정된다.** 로컬에서 `eas build --profile production`을 돌리면 값이 없어 기본값 `development`로 떨어지고, CLI가 패키지를 `com.univer.app.dev`로 판단해 **개발용 keystore를 골라 서버로 보낸다.** 서버는 `EAS_BUILD_PROFILE`이 있으니 `com.univer.app`으로 빌드한다. 그래서 **패키지는 맞는데 서명만 다른** AAB가 나왔다.
  - **수정**: `eas.json`의 production·preview 프로필 `env`에 `APP_VARIANT`를 넣었다. 프로필의 `env`는 로컬 설정 평가에도 적용되므로 양쪽이 같은 값을 본다.
  - 재빌드 후 인증서(`60:E5:…`)·패키지(`com.univer.app`)·버전(`1.0.1`)을 파일에서 직접 확인하고 올렸다.
  - 🔸 **교훈**: 안전 장치를 넣을 때 "값을 못 읽으면 개발용으로"라는 방향 자체는 맞았지만, **로컬에서 그 값이 애초에 안 들어온다는 것을 확인하지 않았다.** 프로필별로 실제 실행해 보는 검증을 했어야 했다.
- **iOS 1.0 출시 처리** — 심사 통과 후 수동 출시로 멈춰 있던 것을 내보냈다. 상태 `1.0 배포 준비됨`. 스토어 반영까지 최대 24시간(확인 시점 `itunes.apple.com/lookup` 아직 비어 있음 — 정상).
  - **출시와 홍보를 분리하기로 했다.** 첫인상을 쓰는 순간은 스토어에 올라간 때가 아니라 사람들한테 깔라고 말하는 때다. 이름 없는 신규 앱은 검색 유입이 사실상 없다. 반대로 안 내면 **크리에이터 포섭이 막힌다** — 데려올 사람 상당수가 아이폰인데 보여줄 물건이 없다.
  - **아무나 가입 못 하는 것을 코드로 확인했다.** DB 트리거 `handle_new_user`가 이메일 도메인을 활성 학교(`kookmin.ac.kr`)와 대조하고, 안 맞으면 `signup_allowlist`를 본 뒤 없으면 `Signup blocked` 예외로 **가입을 거부**한다. 화면 검사가 아니라 서버라 우회가 안 된다.
  - 🔸 리뷰는 남는다. 콘텐츠가 쌓일 때까지 알리지 않고, 직접 접촉한 사람만 쓰는 상태로 둔다.
- 🔴 **테스트 계정(`qa_onboard_test@kookmin.ac.kr`)이 탈퇴 처리돼 로그인이 막혔다 (복구 완료)**
  - 증상: 개발 빌드에서 로그인해도 로그인 화면에 그대로 머문다. **Supabase 인증 자체는 성공**한 것을 `last_sign_in_at`으로 확인했고, `public.users`의 `deleted_at`이 9/6 13:22로 채워져 있었다. `app/login.tsx`는 세션이 잡혀야 넘어가는데, 탈퇴 계정은 세션 프로바이더가 즉시 로그아웃시킨다.
  - **앱 안에서 탈퇴 버튼이 눌린 것이 맞다.** 같은 시각(소수점까지 동일)에 게시물 2건·메시지 6건이 함께 삭제 표시됐다 — `delete_account` RPC가 한 번에 하는 일이다. 이 함수는 `auth.uid()` 기준이라 **본인만 자기 계정을 지울 수 있다.** 관리자도 이걸로는 남의 계정을 못 지운다.
  - **복구**: 탈퇴 시각과 정확히 일치하는 행만 되돌렸다(계정 1·게시물 2·메시지 6). 댓글·좋아요·북마크는 `delete_account`가 완전히 지워서 복구 불가.
  - 🔸 **누가 눌렀는지는 확정 못 했다.** `auth.audit_log_entries`가 비어 있어(보관 기간 경과) 로그인 출처를 못 본다. **Play 사전 출시 보고서의 자동 테스트 로봇**일 가능성이 있다 — 13:09 제출 직후인 13:22에 벌어졌다. Play Console → 테스트 및 출시 → 사전 출시 보고서에서 실행 기록과 화면 캡처로 확인 가능. 테스트 계정 정보를 넣어뒀다면 **새 버전마다 반복된다.**
  - 🔸 **탈퇴 확인을 강화해야 한다(미작업).** 지금은 확인창에서 두 번 누르면 끝난다. 비밀번호 재입력은 못 쓴다 — **구글로 가입한 사용자는 비밀번호가 아예 없다.** 대신 **닉네임을 직접 입력**하게 하는 방식을 쓰기로 했다. 로그인 방식과 무관하게 통하고, 실수도 막고, 자동 테스트 로봇도 닉네임은 모른다.
  - 🔸 **탈퇴한 계정으로 로그인하면 앱이 아무 말 없이 로그인 화면에 멈춘다.** 실제 사용자에게도 벌어질 수 있다. "탈퇴한 계정입니다" 안내가 필요하다.

## 2026-09-04

### 완료
- **홈피드에 팔로우 반영 + 작성자 상한** — 승격 계정만 팔로우를 받을 수 있는데(8/27) 팔로우해도 그 사람 글이 위로 오지 않아 **크리에이터가 팔로워를 모을 이유가 없었다**(팔로우 총 2건). 밴드 0의 조건을 `안 봤음 AND 크루`에서 `안 봤음 AND (크루 OR 팔로우)`로 넓혔다. 밴드를 따로 만들지 않은 이유는 같은 밴드 안 최신순이면 크루 글과 팔로우 글이 시간 순으로 섞이기 때문이다. 팔로우를 위에 따로 두면 실제로 아는 사람보다 항상 먼저 뜬다.
  - **작성자 상한**: 밴드 0에는 한 사람당 최신 2개까지만 넣는다(`row_number() over (partition by user_id order by created_at desc) <= 2`). 셋째 글부터는 밴드에서 빠져 원래 자리(6시간 내 → 밴드 1, 30일 내 → 밴드 2, 그 외 → 밴드 4)로 내려간다. 삭제가 아니라 밀어내기라 글을 몰아 올린 크리에이터도 노출을 잃지 않는다.
  - **검증**: `badbadtzmaru`(`s2j_3` 팔로우) 기준으로 s2j_3의 안 본 글 2개가 밴드 0 최상단에 올라왔고, 나머지는 나이에 따라 밴드 3·4로 내려간 것을 전수 확인했다. 팔로우가 없는 `tester_daan`은 결과가 그대로다. 함수 1개·`anon`/`authenticated` 실행 권한 유지 확인.
  - 시그니처가 그대로라 `CREATE OR REPLACE`를 썼다(DROP하지 않아 권한이 안 사라진다). 마이그레이션 `20260904150000_feed_follow_band_and_author_cap`, 원격 적용·검증 완료. **앱 수정 불필요** — 밴드 개수가 0~4로 그대로고 `HomeFeedList`의 `>= 3` 판정도 안 바뀐다.
- **탐색 정렬에 기본 점수와 크리에이터 가산점 추가** — 두 가지를 넣었다. ① 기존 식이 `(좋아요 + 댓글×2)`라 **반응 0인 글은 점수도 0**이어서 갓 올라온 글이 무조건 맨 아래로 갔다. 홈 피드는 이미 `1 +`가 있었는데 탐색만 빠져 있었다. ② 승격·기관 계정에 **1.5배 가산점**을 준다. 탐색은 "모르는 사람을 발견하는 자리"라 크리에이터를 밀어줘도 홈의 "내 사람들 소식" 감각을 해치지 않는다.
  - **배수 결정 근거(실측)**: 유일한 승격 계정 `s2j_3`의 최고점 글이 7위(3.894)였는데 1.5배로 **1위(6.675)**가 된다. 2~8위는 여전히 일반 학생이라 화면을 지배하지 않는다. 2배는 과해 일반 학생 글이 밀린다. 배수는 `p_creator_boost` 인자로 빼 조절 가능.
  - 호출부는 앱 `features/explore/api.ts` 한 곳뿐이고 인자를 넘기지 않아 기본값을 쓴다. 웹에는 호출부가 없다. 앱 수정 불필요.
  - 마이그레이션 `20260904140000_explore_base_score_and_creator_boost`, 원격 적용·검증 완료.
  - 🔸 **배경 — 크리에이터 포섭 준비**: 릴스 조회가 일주일간 사실상 0이다(9/4~8/28 중 8/29의 2건이 전부). 영상은 `ready` 상태로 피드 노출도 되고 있어 인코딩 문제가 아니라 **릴스 탭에 아무도 안 들어가는 것**이다. 영상이 10개뿐이라 랭킹으로 풀 문제가 아니고 올리는 사람을 늘려야 한다. 노션 크리에이터 후보 풀에 대상이 정리돼 있으나 접촉 전이며, 데려오기 전에 크리에이터에게 줄 이점이 실제로 작동해야 한다.
  - 🔸 **남은 것 — 팔로우가 피드에 반영되지 않는다.** 승격 계정만 팔로우가 가능한데 팔로우해도 그 사람 글이 위로 오지 않아, 크리에이터가 팔로워를 모을 이유가 없다. 홈 밴드 0(크루)에 팔로우를 합치고 그 안에서 최신순, 나아가 크루/팔로우를 번갈아 배치하는 방식을 검토했다(`follows` 테이블 상태 확인 필요).
- **앱 스토어 업데이트 안내** — 로그인·온보딩 완료 후 플랫폼별 최신 버전을 하루 한 번 조회하고, 현재 앱보다 새 버전이면 공용 확인 다이얼로그로 스토어 이동을 권유한다. 닫은 서버 버전 문자열을 저장해 같은 버전은 다시 띄우지 않고 다음 버전은 다시 안내하며, `min_version` 설정 시 취소·바깥 탭·Android 뒤로가기를 막는 강제 모드도 준비했다. 조회·스토리지 실패는 선택 업데이트에서 앱 사용을 막지 않는다.
  - 마이그레이션 `20260904072009_app_release_notice` (테이블 `public.app_release`: platform PK / latest_version / min_version / store_url / message). RLS는 `authenticated` SELECT만 열려 있고 쓰기 정책이 없어 일반 사용자는 변경할 수 없다. 원격에만 적용돼 있던 것을 로컬 파일로 내려받아 저장했다.
  - 🔸 **실기기 확인 때 안 뜨던 원인은 하루 한 번 조회 제한이었다.** DB를 1.0.1로 올리기 전에 앱이 이미 조회해 1.0.0을 저장해뒀고, 24시간 동안 다시 묻지 않아 옛 값으로 비교하고 있었다. 앱 정보에서 *캐시* 삭제로는 안 지워지고 *데이터* 삭제(AsyncStorage 초기화)를 해야 재조회한다. 로그를 찍어 값으로 확인했고 확인 후 로그는 제거했다. 실제 배포 시에도 스토어에 새 버전이 깔린 뒤 최대 하루까지 늦게 뜰 수 있다(의도된 동작).
  - 🔸 확인용으로 android `latest_version`을 1.0.1로 올렸다가 **1.0.0으로 되돌려 뒀다.** 실제 갱신은 AAB가 심사 통과·배포된 뒤에 해야 한다. 먼저 올리면 아직 없는 버전으로 사용자를 스토어에 보내게 된다.
- **본 글 재노출 창 3일→7일, 새 글 없을 때 열람 마커 숨김** — 실기기에서 피드 맨 위에 바로 "모두 열람했습니다"가 뜨는 것을 확인했다. 원인 둘. ① 재노출 창이 3일이라 대상이 20개뿐이라 활동적인 계정은 **첫 글부터 이미 본 글**이 된다. ② 마커는 "여기까지가 새 글"이라는 구분선인데, 새 글이 0개면 맨 위에 붙어 구분선이 아니라 "볼 게 없다"는 통보가 된다.
  - 재노출 창을 **7일**로 넓혔다(3일 20개 → 7일 47개, 14일 51개라 7일이 적정선). 마이그레이션 `20260904120000_feed_reshow_window_7days`, 원격 적용 완료.
  - `HomeFeedList`에서 **첫 글부터 본 글이면 마커를 아예 넣지 않도록** 했다. 구분할 새 글이 없을 때는 구분선도 없는 게 맞다.
- **본 글 재노출 밴드를 셔플로 전환** — 새로고침해도 화면이 그대로라 반응이 없어 보이던 문제. 새로고침은 정상 동작하고(`handleRefresh`가 `ignoreCache: true`로 서버 재조회 + 새 seed 생성) 스피너도 돌지만, **정작 seed를 쓰는 밴드가 4(한 달 넘은 글)뿐이라 화면에 안 나왔다.** 나머지는 최신순·핫스코어라 새 글이 올라오지 않는 한 순서가 고정된다. 이미 본 글은 순서에 의미가 없으므로 밴드 3도 seed 셔플로 바꿨다. 확인 결과 seed를 0.7→0.2로 바꾸면 상위 8개 중 7개가 자리를 옮긴다. 마이그레이션 `20260904110000_feed_shuffle_reshown_band`, 원격 적용·검증 완료. 앱 수정 불필요.
  - 🔸 셔플은 콘텐츠가 적을 때의 임시방편이다. 같은 글을 계속 보게 되는 건 셔플 탓이 아니라 하루 6~9개뿐이기 때문이고, 셔플을 안 하면 같은 글을 **같은 순서로** 보게 된다. 게시물이 늘면 밴드 3까지 내려갈 일이 없어져 저절로 안 쓰이게 된다.
- **"모두 열람했습니다" 마커 위치 수정** — 밴드를 재구성하면서 깨진 것을 바로잡았다. `HomeFeedList.tsx`가 `band === 2`를 "본 글의 시작"으로 못박고 있었는데, 밴드가 `0 크루 / 1 신규 / 2 안 본 최근 글 / 3 본 글 재노출 / 4 그 외`로 바뀌면서 **아직 안 본 글 앞에 마커가 붙었다.** 첫 재노출 밴드 이상(`>= 3`)에서 넣도록 고치고, 특정 숫자로 못박지 않는 이유를 주석으로 남겼다.
  - 🔴 **놓친 경위**: 밴드 설계 때 "클라이언트는 band를 커서로 되돌려줄 뿐이라 앱 수정 불필요"라고 단언했는데, 확인 범위를 `features/feed/`로만 잡고 화면 컴포넌트를 안 봤다. 실제로 숫자를 직접 판단하는 곳이 `HomeFeedList.tsx` 한 군데 있었다. 이번엔 `src` 전체를 훑어 `band`를 값으로 비교하는 곳이 그 한 곳뿐임을 확인했다.
