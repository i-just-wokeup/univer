...
- **피드/모바일 게시물 상세 카드 룩 통일**
  - 홈 피드 `PostCard`를 16px 둥근 흰 카드, 좌우 인셋, 그림자 없는 표면으로 정리
  - 피드 카드 최상위 패딩을 제거해 게시물 이미지가 카드 폭을 풀블리드로 채우도록 보정
  - 모바일 게시물 상세를 피드와 같은 카드 구조로 재배치하고, 이미지 풀블리드/액션·본문 패딩 구조를 맞춤
