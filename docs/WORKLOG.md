...
- **앱 제한 미디어 권한 안내** — 사진·영상 보관함이 `선택한 항목만 허용` 상태일 때 그리드 마지막에 기존 빈 상태와 같은 세로형 안내를 표시했다. Android의 재요청 창은 앱별 권한 상태에 따라 전체 허용 선택지가 보장되지 않고 Expo SDK 54의 재선택 API도 같은 네이티브 요청 경로를 사용함을 실제 개발 APK·Play AAB와 패키지 소스로 확인했다. 이에 Android는 `권한 설정 열기`로 앱 설정에서 전체 허용을 직접 선택하게 하고, 복귀 시 권한을 재조회해 목록·페이지 커서를 초기화한다. iOS는 기존 항목 재선택 시트를 유지한다. 앱 tsc 통과, 순수 JS라 재빌드 불필요.
