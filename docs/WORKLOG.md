...
### 완료
- **홈피드 밴드를 글 나이 기준으로 세분화 (재노출 + 오래된 글 밀어내기)** — 하루 게시물이 6~9개뿐이라 활동적인 테스터는 안 본 글이 0개가 되어 피드가 단조로워지는데(소진율 100% 2명), 반대로 안 본 글에는 나이 제한이 없어 4~7월 개발 중 테스트 글 29개가 핫스코어 밴드에 그대로 섞여 있었다. 볼 게 없으면 몇 달 전 글이 올라오는 구조였다. 밴드를 `0 크루 / 1 신규 / 2 최근 30일 안 본 글 / 3 최근 3일 본 글(재노출) / 4 그 외` 로 나눴다. 본 글은 **본 시각이 아니라 글이 올라온 시각** 기준으로 되살린다. 한 달 넘은 29개는 밴드 4로 밀려나 사실상 노출되지 않는다. 삭제가 아니라 순서만 뒤로 보내는 방식이라 되돌리기 쉽고, 콘텐츠가 늘면 밴드 3·4까지 내려갈 일이 없어 저절로 물러난다. 인자 `p_recent_days`(30)·`p_reshow_days`(3)로 조절 가능. 마이그레이션 `20260904100000_feed_age_bands_and_reshow`, 원격 적용·검증 완료(함수 1개·권한 복원 확인). 앱 수정 불필요.
  - 🔸 **"봤다" 기준은 건드리지 않았다.** 이미 `화면 80% 노출 + 2초 이상`(`feedViewability.ts`)으로 충분히 엄해 조정 효과가 작고, 재노출 효과와 섞이면 무엇이 통했는지 구분이 안 된다.
  - 🔸 **인스타 참고**: Mosseri가 밝힌 주요 신호는 watch time · sends per reach · likes per reach로 개수가 아니라 **노출 대비 비율**이다. 우리는 개수 기반(`1 + 좋아요 + 댓글×2`)이라 노출을 많이 받은 글이 유리하다. 비율 전환은 콘텐츠가 늘어난 뒤 검토한다. 지금은 신규 밴드로 새 글이 경쟁 자체를 피하게 해 해결했다.
  - 🔸 4~7월 테스트 글 삭제는 보류. 실제 테스터 글이 섞였는지 확인 후 별도 판단한다.
...
### 완료
- **개발 빌드 패키지명 분리 (`app.config.js` 도입)** — 개발 빌드가 Play 스토어 앱과 같은 `com.univer.app`이라 설치하면 스토어 앱을 덮어썼다. 본인 계정이 테스터 12명 중 하나라 앱이 지워지면 14일 요건이 위험해 실기기 확인을 못 하고 테스터 배포가 곧 검증이 되는 상황이었다. `app.json`은 그대로 두고 `app.config.js`가 빌드 프로필에 따라 값만 덮어쓰도록 했다(Expo 공식 권장 방식). 개발 빌드는 `com.univer.app.dev` / 앱 이름 `unip (dev)`로 나가 스토어 앱과 나란히 설치된다.
  - **실패 방향을 안전한 쪽으로 잡았다** — 환경변수를 못 읽으면 프로덕션이 아니라 개발용 패키지로 떨어진다. 반대로 두면 변수 하나 누락으로 개발 빌드가 스토어 앱을 덮어쓴다(EAS 환경변수 누락은 2026-08-27에 실제로 겪었다). 프로덕션 경로에서는 패키지명이 어긋나면 즉시 예외를 던진다. 프로필별로 실행해 production·preview가 기존 값 그대로임을 확인했다.
  - **개발 빌드에서는 `googleServicesFile`을 뺐다** — 그 파일이 `com.univer.app` 하나만 담고 있어서다. 설치된 `@expo/config-plugins`의 `GoogleServices.js`를 직접 읽어 확인한 결과 이 항목이 없으면 google-services Gradle 플러그인 자체를 붙이지 않으므로 빌드는 정상 통과한다. 대신 개발 빌드에서 푸시와 구글 로그인은 동작하지 않으므로 이메일 계정으로 로그인해 확인한다.
  - 앞으로는 `npx expo start --dev-client`로 폰에 붙여 저장 즉시 확인하고, 검증된 것만 `eas update`로 테스터에게 내보낸다.
- **앱 제한 미디어 권한 안내** — 사진·영상 보관함이 `선택한 항목만 허용` 상태일 때 그리드 마지막에 기존 빈 상태와 같은 세로형 안내와 `권한 다시 선택` 버튼을 표시했다. Android 14 이상은 공식 권장 방식대로 `READ_MEDIA_IMAGES`/`READ_MEDIA_VIDEO`와 `READ_MEDIA_VISUAL_USER_SELECTED`를 `PermissionsAndroid.requestMultiple`로 한 번에 직접 요청한다. iOS는 기존 항목 재선택 시트를 유지하며, 권한 창 종료 후 상태를 재조회하고 목록·페이지 커서를 초기화한다. 앱 tsc 통과, 현재 Play AAB에 권한 선언이 포함된 JS 변경이라 재빌드 불필요.

## 2026-09-02

### 완료
- **앱 영상 미리보기 버퍼 상한 상향 (8MB → 16MB)** — `StoryVideoView`의 로컬 파일 분기가 `maxBufferBytes: 8MB`와 `preferredForwardBufferDuration: 5초`를 함께 걸어 비트레이트 높은 편집앱 영상이 재생 시작 조건을 못 채우고 `Playback stuck buffering and not loading`으로 첫 프레임에서 멈췄다(ADB logcat으로 확정). 상한만 16MB로 올려 모순을 없앴고, 미리 읽는 시간 5초 설정이 남아 실제 메모리는 그보다 훨씬 아래로 유지된다. 8MB 캡은 2026-06-30 릴스 OOM을 막으려던 임시 방어의 잔재이며, 릴스·피드는 별도 파일(`useReelVideoPlayer`·`FeedVideoPlayer`)이라 이번 변경에 닿지 않는다. 왜 8MB가 문제였는지 주석으로 남겼다. 앱 tsc 통과, 순수 JS.
- **테스터 피드백 3건 접수** — ① 사진 선택에서 기기 사진이 안 보임(제보 2건째, 원인은 안드로이드 제한 권한 + 안내 누락) ② 카메라 사진이 안 나오고 캡처만 먼저 나옴(8/31 커밋 `83a2fc6`으로 이미 수정됐으나 배포가 안 돼 계속 겪던 건) ③ 게시물 작성에서 바로 촬영 요청(기능, 릴리스 3 후보). 노션 「출시 운영」 피드백 로그에 기록했다. **고쳐두고 배포하지 않으면 반영되지 않는다**는 것이 이번 교훈이다.
- **앱 인사이트 미표시 지표 노출 (Codex)** — 개요 타일에 `링크 클릭`을 추가했다. `recordMetric("link_click")`은 정상 기록되고 있었는데 `useInsights`가 `post_view`·`profile_visit`만 조회해 항목 자체가 화면에 없던 문제다. 함께 `프로필 방문`이 일반 계정에만 보이던 분기를 풀어 승격·기관 계정에서도 보이게 했다. 타일이 최대 5개가 되면서 `InsightsOverview`에 `flexWrap`을 주고 타일을 `flex:1`에서 `flexBasis:88 + flexGrow:1`로 바꿔 좁은 화면에서 찌그러지지 않고 줄바꿈되도록 했다. 조회 유형 라벨도 `릴스`→`릴스 조회`처럼 무엇을 세는 값인지 드러나게 정리했다. `link_click`은 DB CHECK 제약에 이미 포함된 값이라 스키마 변경은 없다. 앱 tsc 통과, 순수 JS라 재빌드 불필요.
- **앱 인사이트 릴스 조회·프로필 링크 클릭 노출 보강** — 기간별 인사이트가 기존에 수집하고도 화면에서 누락하던 `link_click`을 조회해 링크 클릭 타일과 일별 추이·이전 기간 증감을 표시하도록 연결했다. 기관·승격 계정에서도 프로필 방문을 함께 보이게 했고, 늘어난 지표 타일은 모바일 폭에서 줄바꿈되도록 보정했다. 콘텐츠 유형별 조회 항목은 릴스·게시물·스토리 조회로 명확히 표기했다. 원격 집계에서 실제 릴스 조회와 링크 클릭 반환을 확인했고 앱 tsc를 통과했다. 본인 콘텐츠 조회·본인 링크 클릭을 제외하는 서버 정책은 유지했다.

## 2026-09-01

### 완료
- **앱 크루 용어 통일** — 같은 프로필 화면에서 통계는 `크루`인데 액션 버튼은 `친구 신청`이던 혼용을 정리했다. 프로필 액션 버튼·더보기 메뉴·알림 문구·연결 상태 및 신청 실패 토스트까지 사용자에게 보이는 문자열을 모두 `크루`로 맞췄다. 함께 섞여 있던 `신청`/`요청`도 `신청`으로 통일해 크루 관리 탭(`받은 신청`·`보낸 신청`), 빈 목록 안내, 삭제 확인 문구, 대기 상태 버튼(`신청됨`)을 정렬했다. 채팅의 `메시지 요청`은 별개 개념이라 그대로 두었다. 웹(`src/`)의 알림 패널·프로필 페이지에 남은 `친구` 표기는 후속으로 미뤘다. 순수 JS라 재빌드 없이 OTA로 나간다. 앱 tsc 통과.

## 2026-08-31

### 완료
- **앱 게시물 미디어 선택 목록 저장 최신순 정렬** — SDK 54 `expo-media-library`가 Android·iOS 모두 지원하는 `modificationTime` 내림차순으로 사진·영상 보관함 조회를 변경해 카카오톡 등 외부 앱에서 받아 저장한 미디어가 원본 촬영일 위치로 밀리지 않고 상단에 나타나게 했다. 전체 보관함과 앨범별 목록·앨범 커버가 동일 조회 함수를 사용하며 기존 `endCursor`/`after` 페이지네이션 계약은 유지했다. 스토리의 `expo-image-picker` 경로는 변경하지 않았다.
