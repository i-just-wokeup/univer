...
### 완료
- **앱 스토리 시작 스토리 인덱스/영상 터치 레이어 수정** — 스토리바는 최신 스토리 썸네일을 보여주는데 뷰어는 유저 그룹의 0번째 스토리부터 시작하던 구조를 수정. 스토리바 → 라우트/단기 세션으로 `storyId`를 함께 전달하고, `StoryViewerScreen`/`StoryPlayer`/`useStoryPlayer`가 `initialStoryIndex`를 받아 해당 스토리부터 시작하도록 변경. 여러 개 올린 스토리에서 왼쪽 탭이 이전 스토리로 가지 않고 진행바만 초기화되는 문제를 막기 위해 인덱스 범위도 clamp 처리. Android `expo-video` 네이티브 뷰가 좌/우 탭을 가로챌 수 있어 스토리 미디어 프레임은 터치 비활성화하고 탭존 elevation을 보강. 앱 tsc 통과.
- **앱 스토리 뷰어 세션/탭존 추가 보정** — 내 스토리 진입 시 다른 유저 스토리가 보이는 문제를 막기 위해 홈 스토리바 세션은 저장된 시작 유저와 라우트 유저가 정확히 같을 때만 재사용하도록 강화. 라우트 파라미터 배열 케이스를 문자열로 정규화하고, 시작 유저를 못 찾으면 다른 유저 0번 그룹으로 fallback하지 않고 닫히게 수정. 전체 화면 일시정지 Pressable이 좌/우 탭을 간섭할 수 있어 pause 영역을 가운데로 제한하고 좌/우 탭존 zIndex를 명시해 이전/다음 이동을 분리. 앱 tsc 통과.
