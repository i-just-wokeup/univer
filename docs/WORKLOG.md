...
### 완료
- **앱 게시물 리셰어 스토리 카드 미디어 완성** — 원본 게시물 조회에 `aspect_ratio`를 포함하고 공용 `StorySharedPostMedia`로 사진·영상을 같은 박스 규칙으로 렌더한다. 세로 4:5와 정사각은 원본 비율을 유지하고, 가로 16:9는 정사각 검정 레터박스 안에 contain으로 표시한다. 카드는 앱 라이트/다크 테마와 무관하게 fixed `white`/`black` 토큰만 사용하는 흰 카드로 통일했다. 영상은 작성 미리보기와 활성 스토리에서만 소스를 연결해 무음 자동재생·반복하며, 공유 영상 스토리는 최대 15초 세그먼트로 진행한 뒤 다음 스토리로 이동한다. 비활성 보관함 미리보기는 썸네일만 표시해 동시 디코딩과 전송량을 막았다. 앱 tsc 통과, 기존 DB 스키마만 사용하므로 추가 마이그레이션·네이티브 리빌드는 없다.
