...
### 완료
- **앱 스토리 닫기 잔상 완화** — 스토리 종료/닫기 시 `router.back()`을 즉시 호출하지 않고, `StoryPlayer` 내부 미디어를 먼저 검정 화면으로 비운 뒤 다음 프레임에 라우트를 닫도록 변경. 기본 Stack 전환 애니메이션은 유지하면서 `expo-video`/이미지 native surface가 피드 위에 한 프레임 사각 잔상으로 남는 현상을 줄임. 앱 tsc 통과.
