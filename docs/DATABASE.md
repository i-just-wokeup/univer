...
- `request_promotion()`(2026-08-14)은 활성·미탈퇴 일반 계정이 삭제되지 않은 게시물 10개 이상과 최근 30일 5개 이상을 충족할 때 pending 승격 신청을 만든다. 이미 승격/기관 계정, pending 중복, 최근 거절 후 7일 이내 재신청은 서버에서 거부한다. `get_promotion_requests_for_admin()`은 활성 관리자만 pending 신청자의 게시물·조회·피드 도달 고유 계정·참여·영상·평균 완주율 성적표를 조회할 수 있다. `approve_promotion`/`reject_promotion`은 pending 행을 잠근 뒤 처리 상태와 담당 관리자를 기록하고 승인 시 `users.is_promoted=true`, 양쪽 모두 신청자 알림을 생성한다. 네 RPC는 SECURITY DEFINER, `search_path=''`, `authenticated` 전용이며 `PUBLIC`/`anon` 실행 권한을 회수한다. 마이그레이션 `20260814070945_promotion_requests`. 자격의 최근 30일 하한은 이후 3→5로 상향(마이그레이션 `20260814090000_promotion_require_5_posts_30d`).
