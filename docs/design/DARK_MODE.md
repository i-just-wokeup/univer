...
> **✅ 현황(2026-08-03): 전 일반 화면 다크 전환 완료** — Phase 0~4(홈·프로필·채팅·검색·탐색·알림·작성·상세·설정·활동·인사이트·인증) 전부 반영. 팔레트는 딥다크로 튜닝(배경 `#0F1011`, 카드 `feedCard` `#000`, 글씨 메타 그레이 `#E4E6EB`/`#B0B3B8`/`#8A8D91`) — **최신 값은 `theme.ts`가 정본**(아래 표는 초안). 미디어(릴스/스토리)는 fixed라 대상 아님. **남은 것: 수동 라이트/다크 토글(설정 스위치+저장).**
