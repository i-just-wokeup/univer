...
### 영상 삭제 접수 관찰 (2026-09-16)

- DB에 근거가 있는 후보만 관찰한다. 실제 Cloudflare 삭제/HTTP/크론/업로드 대장은 이번 범위 밖이다. 스토리 만료는 보관함 삭제가 아니다.
- 영상 식별자와 삭제 사건을 분리한다. soft→hard는 기존 사건 보강, 복구는 취소, 재삭제는 generation 증가. 원본/사용자 FK를 두지 않아 CASCADE에도 기록이 남는다.
- 원본 삭제를 우선한다. 접수 변경 실패는 하위 트랜잭션으로 격리하고 식별자·SQLSTATE만 WARNING에 남긴다. BEFORE DELETE는 항상 OLD 반환. 같은 원본 동시 접수는 try-advisory lock, 실패하면 경고하며 누락 가능성을 숨기지 않는다.
- 사유는 content_soft_delete/source_hard_delete/reference_changed라는 관측 사실뿐. 사용자 변경 순서로 계정 탈퇴라고 추론하지 않는다. 계정 ID는 원본에 없어 NULL이며 향후 실제 계정 검증 필수.
- eligible_after는 NULL만 허용한다. 약관의 탈퇴 30일을 개별 게시물 유예로 확대하지 않으며 정책 승인 없이 실행 가능 상태로 만들지 않는다.
- 접수증은 취소 후 90일 보존 뒤 수동 정리 대상. 미해결 pending/held는 유일한 삭제 근거 보존을 위해 해결까지 유지하되 90일마다 필요성과 식별자 최소화를 검토한다. 자동 정리는 이번에 없으며 영상 보존 정책과 별개다.
- 관리자 읽기는 RLS SELECT로만 제공한다. 일반 사용자 쓰기와 내부 helper 실행은 차단한다. 상세 한계·조회·테스트: [STREAM_DELETION_RECEIPTS.md](./STREAM_DELETION_RECEIPTS.md).

### 관리자 성장 지표 (2026-09-16)

- 잔존은 D1=1일차, D7=5~7일차, D30=28~30일차. 소규모 코호트에서 하루 차이로 잔존이 과도하게 출렁이지 않게 한다. D1만 가입 당일을 포함하지 않도록 1일 창으로 유지한다. 마지막 판정 날짜가 끝난 사람만 분모에 넣는다.
- 첫 반응 시간은 평균이 아닌 중앙값. 한참 뒤에 붙은 반응의 이상치 영향을 줄인다. 자기 좋아요/댓글은 반응에서 제외한다.
- 무반응률은 KST 오늘 포함 30일의 모든 미삭제 글로 계산하며 새 글을 24시간 유예하지 않는다. 재작성은 전체 기간 미삭제 첫 글 이후 168시간 관찰 완료자만 대상으로 한다.
- 주간 대표는 글 작성 주 기준, 이번 주 포함 8주이며 조회 시점까지 받은 타인 반응을 센다. 과거 주도 반응 추가/취소에 따라 변할 수 있다.
- 주간 추이 막대는 동일 최대값, 왼쪽 기준선, zinc-200 트랙, 분리된 축을 사용한다. 비교 가능한 크기와 단색 운영 화면을 유지한다.
...
- "인스타 트렌딩 음원은 API로 못 붙인다" → **2026-05 메타가 3자 배포 승인 트랙만 개방.** 반만 맞음(2026-09-16 정정)
...
**결정:** 우리 앱에서 인스타그램으로 같이 올려주는 기능은 만들지 않는다.

**이유 (2026-09-16 실제 확인):**
- 인스타 콘텐츠 게시 API는 **프로페셔널 계정만** 쓸 수 있다. 일반 학생 계정은 아예 못 붙는다. 우리 타겟 대부분이 여기서 잘린다.
- `instagram_business_content_publish` 권한 심사에 2~4주. 24시간 100건 제한.
- 음원: **2026년 5월에 메타가 음원 라이브러리 일부를 외부 도구에 열었다.** 이전 기록의 "트렌딩 음원은 API로 못 쓴다"는 이제 반만 맞다. 다만 열린 건 3자 배포가 승인된 트랙뿐이라 인스타 앱 안 목록보다 훨씬 좁다.
- 올린 직후 몇 시간이 분류에 중요한데 음원을 나중에 붙이면 그 구간을 놓친다. 그래서 실무에서는 **"도구로 예약 → 알림 받고 손으로 발행"** 반자동이 흔하다.

### 사진앱 공유하기로 받기(share intent) — 하지 않는다

**결정:** 다른 앱의 "공유하기"로 사진을 받아 게시물을 만드는 기능은 만들지 않는다.

**이유:** 단계 수가 줄지 않는다. 사진앱 열고 → 공유 → 우리 앱 고르기나, 우리 앱 열고 → 글쓰기 → 사진 고르기나 같다. 네이티브 설정이라 빌드도 다시 해야 한다.

### 인스타 콘텐츠 가져오기 — 방식은 Meta 데이터 이전, 단 지금은 보류

**결정:** zip 파일 임포트는 막혔고(인스타가 비밀번호를 계속 거부), 제대로 된 경로는 **Meta Export Your Information(EYI) 목적지로 등록**하는 것이다. 다만 지금은 착수하지 않는다.

**확인한 요건 (2026-09-16):**
- DTI 파트너(Amazon·Apple·Google·Meta·Ernie, 연 1회 심사 + 회비)와 **DTI 신뢰 등록소는 다르다.** 우리가 필요한 건 등록소고, 아무 회사나 신청 가능하다.
- **레벨 1** — 법인, 서비스 설명 홈페이지, 개인정보처리방침, 보안 제보 창구. 받을 수 있는 건 **사진·영상·게시물**. 우리가 원하는 건 여기까지다.
- 레벨 2 — 외부 보안 감사(SOC 2 등) 필요. 아카이브·blob용. **우리는 필요 없다.**
- 메타 쪽 추가 요건: 인증된 Business Manager 계정, OAuth 2.0 지원, 메타 엔지니어링 파트너와 종단 테스트.
- 비용·심사 기간은 어디에도 공개돼 있지 않다.

**보류 이유:** 우리가 OAuth 2.0 **제공자**가 되어야 하고 DTP 어댑터 엔드포인트를 운영해야 한다. 지금은 Supabase 인증에 얹혀 있어 인증 서버를 새로 세우는 일이다. 그리고 이건 계정 이사용 **일회성 통째 이전**이지 매번 올릴 때 쓰는 게 아니다. 유저 22명 단계의 순서가 아니다.

### 우리 쪽 공개 업로드 API — 나중에 쓸 카드, 지금은 안 만든다

**배경:** Buffer·Metricool·Later·Publer·Repurpose.io 등 **여러 플랫폼에 한 번에 올려주는 도구는 이미 널리 쓰인다.** 다만 그 도구들이 지원하는 목적지는 각 회사가 정하므로, 우리가 인스타에 올리는 건 메타 허락이 필요하지만 **남이 우리에게 올리는 건 우리 허락만 필요하다.**

**지금 안 만드는 이유:**
- 붙을 소비자가 없다. 이 규모에서 Buffer가 우리를 목적지로 넣어줄 리 없다. 누가 붙을지 정해져야 인증 방식이 정해진다.
- 공개 API는 한 번 내면 못 바꾼다. `posts` 스키마가 아직 흔들린다(영상·승격·밴드 계속 변경 중).
- 영상 ingest 를 외부에 열면 Cloudflare Stream 비용 남용 위험이 있다(`stream-upload-url` 엣지 함수가 직접 업로드 URL 을 발급한다).

**나중에 필요해지면 만들 것:** API 키 발급 개념(지금 없음), 안정적인 게시물 생성 계약, 외부용 미디어 ingest 경로, 요청 제한. 참고로 Supabase PostgREST 가 이미 RLS 로 보호되는 REST 를 노출하고 있어, 없는 건 "API" 자체가 아니라 **공개 계약·키·미디어 경로**다.
