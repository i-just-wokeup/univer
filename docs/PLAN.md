...
## 진행 중인 작업
- [ ] 차단 기능 구현 및 차단 관계 필터링
- [ ] 게시물/스토리 공개범위 정리 (전체공개 / 크루공개)
- [ ] 탐색(Explore) 탭 MVP 구현
...
1. 차단 기능 구현
   - 게시물 `...` 메뉴의 차단 연결
   - 피드/검색/프로필/채팅 차단 관계 반영
2. 게시물/스토리 공개범위 정리
   - UI 문구: 전체공개 / 크루공개
   - DB 값: `public` / `close_friends` 유지
   - 크루공개 권한은 `user_connections.status = 'accepted'` 기준
3. 탐색(Explore) 탭 MVP 구현
   - 하단 탭 `카테고리` → `탐색`
   - 같은 학교 public 게시물 3열 그리드
   - 내 게시물/차단 유저 제외
4. 내 활동 보관함 고도화
   - 스토리 보관함 월별 그룹/캘린더 뷰
5. Resend 연동 (비밀번호 재설정용, 이메일 인증은 OFF 유지)
6. 계정 탈퇴/복구 실사용 테스트
7. Expo 앱 전환 준비
