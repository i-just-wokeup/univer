...
- [x] **일반 학생 스토리 안내 + 홈 첫 진입 코치마크** — 스토리가 없는 일반 학생도 홈 헤더 아래 40px 빈 스토리 영역을 유지하고, 사용자별 첫 홈 진입 한 번만 스토리바·검색·작성·탐색을 순서대로 안내. Context ref 레지스트리 + `measureInWindow` + 순수 JS 4조각 scrim 오버레이로 구현해 네이티브 마스크·추가 라이브러리 없이 처리
...
- [ ] **승격 계정 인사이트 확장 (A~E)** — 크리에이터로 전환 유도할 심화 지표. 각 지표 준비되면 승격 계정 인사이트 화면에 붙이고 `canViewInsights` 게이트로 승격 전용 노출.
  - [~] **A. 콘텐츠 성과** — 게시물별 좋아요·댓글·저장·공유. `get_content_performance()` RPC + `getContentPerformance()` api·타입 완료(커밋 `bbcc38c`), **UI·게이트 미구현**
  - [ ] **B. 팔로워 인구통계** — 학교·학과·성별 등 팔로워 분포. 팔로우 도입 이후
  - [ ] **C. 팔로우 전환 퍼널** — 프로필 방문 → 팔로우 전환율. 팔로우 도입 이후
