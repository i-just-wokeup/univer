...
## 진행 중인 작업
- [ ] 배포 전 필수 항목 점검 (users RLS, Cron Job, 이메일 인증)

## 다음 작업 (순서대로)

1. 스토리 UI/UX 개선 (남은 세부 인터랙션/시각 보정)
3. 좋아요 목록 모달
4. 관리자 액션 범위 확장 (차단/권한 변경/검색 필터 고도화)
5. 배포 준비 (users RLS 재설계, Cron Job, 이메일 인증 설정)
...
### 남은 점검
1. `users` RLS 재설계 및 현재 재귀 이슈 해소
2. 배포 전 이메일 인증/리다이렉트 설정 확인
3. 배포 후 스토리 만료 정리용 Cron Job 구성
