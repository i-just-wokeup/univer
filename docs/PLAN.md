...
- [~] **승격 계정 인사이트 확장 (A~E)** — 일반 계정은 개요만, 기관·승격 계정은 개요/콘텐츠 탭을 노출한다. v1 대시보드와 A 콘텐츠 성과를 연결했고 수집 기반이 필요한 후속 지표는 계속 확장한다.
  - [x] **A. 콘텐츠 성과** — 게시물별 좋아요·댓글·저장·공유를 전체 누적/게시물 행으로 제공하고 인기순·최신순 정렬과 상세 이동을 연결. 드롭다운 기간 선택, 기간별 조회·참여·도달 선택 타일, 단일 추이 차트, 유형별 조회 막대와 기관·승격 게이트까지 완료
  - [ ] **B. 팔로워 인구통계** — 학교·학과·성별 등 팔로워 분포. 팔로우 도입 이후
  - [ ] **C. 게시물 노출 수집** — `post_impression` 수집 기준과 저장 구조를 확정한 뒤 콘텐츠 노출·전환 지표에 연결
  - [x] **D. 영상 시청 기록** — 릴스 활성 재생 세션을 `reel_watch_events`에 멱등 기록하고 완주율·평균 반복재생·평균 시청 깊이·평균 이탈 지점을 기간별 집계. 95% 완주, 85%→15% 루프 판정, 1초 미만 제외, 비활성/언마운트/백그라운드 종료까지 연결. ⚠️ 콘텐츠 탭 지표 UI 연결과 드롭오프 곡선(v2)은 후속
