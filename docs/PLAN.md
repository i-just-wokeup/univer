...
## 진행 중인 작업
- [x] 일반 계정 로그인 가능 여부 확인 — 완료
- [x] 관리자 계정 로그인 및 `/admin` 접근 가능 여부 확인 — 완료
- [ ] DB 마이그레이션 파일 작성 (1단계 17개 테이블)
- [ ] Supabase 마이그레이션 적용

## 다음 작업 (순서대로)

1. 기존 `/posts/write` 라우트 삭제
2. 공개범위 (전교생/크루 2단계) 게시물/스토리 업로드 연동
3. 스토리 UI/UX 개선 (남은 세부 인터랙션/시각 보정)
4. 좋아요 목록 모달
5. 관리자 액션 범위 확장 (차단/권한 변경/검색 필터 고도화)
