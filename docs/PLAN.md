...
- [x] **피드/모바일 게시물 상세 카드 룩 통일** — 홈 피드와 모바일 게시물 상세를 16px 둥근 흰 카드, 이미지 풀블리드, 그림자 없는 구조로 맞추고 모바일 상세 댓글 입력은 기존 `CommentSheet` 진입으로 유지
...
1. KREW 기반 디자인 모바일 확인
   - 홈 피드 카드와 모바일 게시물 상세가 동일한 16px 둥근 카드/풀블리드 이미지/그림자 없는 표면으로 보이는지 확인
