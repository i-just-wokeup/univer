...
- [x] **앱 추천 크루 캐러셀** — 같은 학교의 공통 크루·같은 학과 후보를 우선 배치하고 남는 자리를 화면 세션 seed 기반 랜덤 후보로 채우는 `get_friend_recommendations` RPC와 검색 빈 화면의 가로 카드 캐러셀을 연결. accepted/pending·차단 관계 제외, 카드 즉시 신청/세션 숨기기, 계정 배지·프로필 이동 지원
