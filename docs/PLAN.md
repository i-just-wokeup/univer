...
- [x] **앱 추천 크루 캐러셀** — 같은 학교의 공통 크루·같은 학과 후보를 우선 배치하고 남는 자리를 화면 세션 seed 기반 랜덤 후보로 채우는 `get_friend_recommendations` RPC와 검색 빈 화면의 가로 카드 캐러셀을 연결. accepted/pending·차단·기관 계정 관계 제외, 카드 즉시 신청/세션 숨기기, 계정 배지·프로필 이동 지원
- [ ] **기관 계정 크루 관계 전면 차단** — 기관 프로필에서 크루 신청/수락 UI를 숨기고, 직접 RPC 호출로 우회할 수 없도록 `send_friend_request`·`accept_friend_request`에서도 `official_accounts` 당사자 연결을 거부. 기존 기관 계정 연결 데이터가 있으면 정리 정책 포함
